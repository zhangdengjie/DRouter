...
abstract class AppendRouterTransform @Inject constructor(
    private val androidComponents: AndroidComponentsExtension<*, *, *>,private val variant:Variant
) : DefaultTask() {
    ...
    private fun assembleClassPath(): Queue<File> {
        ...
        classpath.add(taskProvider.get().destinationDirectory.get().asFile)
        return classpath
    }

    private fun packOutputJar() {
        val jarOutput = JarOutputStream(BufferedOutputStream(FileOutputStream(outputJar.get().asFile)))
